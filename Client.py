...
HOST = '127.0.0.1'  # Localhost address for socket connection
PORT = 5555  # Port number for server-client communication
CONFIG_FILE = "chat_config.json"  # File to store theme preferences

# Theme definitions for UI styling
THEMES = {
    "light": {
...
class ChatMessenger:
    """A Tkinter-based chat client for connecting to a server."""
    def __init__(self, root):
        self.root = root
        self.root.title("Messenger - General Chat")  # English title
        self.root.geometry("900x700")
        self.client_socket = None  # Socket for server communication
        self.nickname = None  # User's chosen nickname
        self.private_windows = {}  # Dictionary to manage private chat windows
        self.theme = THEMES["light"]  # Set default theme to light
        self.user_list_data = []  # List of connected users
    ...
    def load_theme(self):
        """Load the saved theme from the config file, default to light if not found."""
    ...
    def setup_ui(self):
        ...
        self.top_label = tk.Label(self.top_frame, text="Messenger - General Chat", font=("Segoe UI", 14, "bold"))  # English title
        self.top_label.pack(side=tk.LEFT, padx=10, pady=5)

        # Main frame to hold left and right panels
        self.main_frame = tk.Frame(self.root)
        self.main_frame.pack(fill=tk.BOTH, expand=True)

        # Left panel for nickname entry and user list
    ...
    def apply_theme(self, animate=False):
        ...
        # Update private windows if they exist
        ...
        # Animate theme transition if requested
        if animate:
    ...
    def connect(self):
        ...
        try:
            ...
            # Switch to dark theme on successful connection
        ...
        while True:
            try:
                ...
                if message == "NICK":
                    ...
                    self.display_message(message, "General Chat")
            except:
    ...
    def display_message(self, message, chat_target):
        ...
        if chat_target == "General Chat":  # English target
            self.chat_window.insert(tk.END, message + "\n")
            self.chat_window.see(tk.END)

    def handle_private_message(self, sender, message):
        """Handle incoming private messages and open a window if needed."""
    ...
    def open_private_window(self, target):
        ...
        if target not in self.private_windows:
            ...
            msg_display = scrolledtext.ScrolledText(private_window, width=40, height=15, font=("Segoe UI", 11), wrap=tk.WORD, borderwidth=0)
            msg_display.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)

            msg_frame = tk.Frame(private_window)
            msg_frame.pack(fill=tk.X, padx=10, pady=10)
            msg_entry = tk.Entry(msg_frame, font=("Segoe UI", 11), borderwidth=1, relief=tk.FLAT)
            ...
            ttk.Button(msg_frame, text="Send", command=lambda: self.send_private(target, msg_entry.get(), msg_display)).pack(side=tk.LEFT, padx=5)  # English button
            ttk.Button(msg_frame, text="Back to General Chat", command=lambda: self.close_private_window(target)).pack(side=tk.LEFT)  # English button

            self.private_windows[target] = {'window': private_window, 'display': msg_display, 'entry': msg_entry, 'frame': msg_frame}
